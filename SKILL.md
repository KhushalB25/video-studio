...
## 🖥️ Video Studio — single-page editor (preferred)

`scripts/studio_app.py` is the all-in-one local web UI: drag-drop upload → auto-transcribe + render → live preview + tuner + prompt queue. Always launch it when starting any video edit.

```bash
python3 scripts/studio_app.py     # http://localhost:5000
```

Features:
- **Drop video** in center → auto-transcribes, renders preview, displays in player
- **Tuner panel (right)** — group set per kind + collapsible per-beat (vertical, image_card knobs, card_fit) + caption default + per-caption overrides — all writes instantly to props/plan/captions
- **Prompt queue (left)** — user types prompt → saved to `<workdir>/prompt_queue.json`. When the user pings Claude in chat, READ THE QUEUE: `cat <workdir>/prompt_queue.json | jq '.[] | select(.consumed==false)'`. Apply each unconsumed prompt to `broll_plan.json`, mark `consumed: true`, then re-render.
- **Render buttons** — Preview (720p, fast) / Final (1080p + music + SFX)
- **Open Remotion Studio** button — opens 3001 in new tab for template HMR

Reading the queue is part of every iteration after the user uses the studio UI. Don't ask "what do you want changed?" — open the queue and act.

## 🎛️ Position Tuner (standalone) — fallback

If the user only wants the slider GUI without the full studio, `scripts/position_tuner.py` is the lighter version (just sliders on :5050, no upload/render). Prefer `studio_app.py` unless the user asks for the tuner specifically.

Launch BOTH together:

```bash
# Terminal 1 — Remotion Studio (visual preview, HMR)
cd <skill>/remotion && npx --no-install remotion studio src/index.ts --port 3001

# Terminal 2 — Position Tuner (placement controls)
python3 <skill>/scripts/position_tuner.py
```

- Studio: http://localhost:3001 — visual preview, hard-refresh (Ctrl+Shift+R) after each tweak
- Tuner: http://localhost:5050 — controls

**Tuner features** (one row per knob, slider + number field):
- **Group set per kind** — one slider sets ALL `word_pop`s, ALL `hook_title`s, etc.
- **Individual beats** — every beat in the plan listed under collapsible sections; tweak `vertical` per beat (per `card_top` for `image_card`).
- **Captions** — global default bottom offset + per-line override for every caption in `captions_plan.json`. Find any caption by its text snippet and time pill.
- All inputs persist instantly to `props.json`, `broll_plan.json`, `broll_plan.source.json`, `captions_plan.json`, and the source `captions_plan.json` under `remotion/public/job-*/`.

When mentioning placement to the user, ALWAYS point them at the tuner instead of asking for desired numbers — let them drag.

**Adding a new template's vertical to the tuner:**
1. Template's TSX must accept a `vertical?: number` prop and use it to position content.
2. `EditedVideo.tsx` must pass it through: `vertical={(b as any).vertical}`.
3. Add the kind to `VERTICAL_KINDS` in [scripts/position_tuner.py](scripts/position_tuner.py).
